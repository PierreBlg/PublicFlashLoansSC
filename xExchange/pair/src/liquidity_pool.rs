...
use crate::contexts::borrow_liquidity::BorrowLiquidityContext;
use crate::errors::*;
    ...
    + pausable::PausableModule
{
    ...
    fn pool_borrow_liquidity( 
        &self,
        context: &mut BorrowLiquidityContext<Self::Api>,
        storage_cache: &mut StorageCache<Self>,
    ) {
        let amount_removed = &context.payment.amount;

        if context.is_first_token {
            storage_cache.first_token_reserve -= amount_removed;
        } else {
            storage_cache.second_token_reserve -= amount_removed;
        };

        context.token_amount_removed = amount_removed.clone();
    }
