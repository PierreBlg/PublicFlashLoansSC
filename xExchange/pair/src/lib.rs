...
use contexts::base::*;
use contexts::borrow_liquidity::BorrowLiquidityContext;
    ...
    + flashloan::FlashLoanModule
{
    ...
    fn init(
        ...
        self.set_fee_percents(total_fee_percent, special_fee_percent, flashloan_fee_percent);
    ...
    #[endpoint(borrowLiquidity)]
    fn borrow_liquidity(&self,
        borrow_asked : EsdtTokenPayment<Self::Api>,
        user_actions_id : u64
    ) {
        let loan_address = self.check_loan_caller_and_return();

        let mut storage_cache = StorageCache::new(self);

        let is_correct_first_token = borrow_asked.token_identifier == storage_cache.first_token_id;
        let is_correct_second_token =  borrow_asked.token_identifier == storage_cache.second_token_id;
        require!(
            (is_correct_first_token || is_correct_second_token)
                && borrow_asked.amount > 0,
            ERROR_BAD_PAYMENT_TOKENS
        );
        require!(
            self.is_state_active(storage_cache.contract_state),
        ...
        if is_correct_first_token {
            require!(
                storage_cache.first_token_reserve > borrow_asked.amount,
                ERROR_NOT_ENOUGH_RESERVE
            );
        };

        if is_correct_second_token {
            require!(
                storage_cache.second_token_reserve > borrow_asked.amount,
                ERROR_NOT_ENOUGH_RESERVE
            );
        };
     

        let mut context = BorrowLiquidityContext::new(
            borrow_asked.clone(),
            is_correct_first_token
        );

        let specific_token_reserve_before_borrow;
        let lp_supply_before_borrow = storage_cache.lp_token_supply.clone();
        
        if is_correct_first_token {
            specific_token_reserve_before_borrow = storage_cache.first_token_reserve.clone();
        } else {
            specific_token_reserve_before_borrow = storage_cache.second_token_reserve.clone();
        }

        self.pool_borrow_liquidity(&mut context, &mut storage_cache);

        drop(storage_cache);

        self.call_deposit_loan(
            loan_address,
            borrow_asked,
            user_actions_id,
            is_correct_first_token,
            specific_token_reserve_before_borrow,
            lp_supply_before_borrow
        );
